"""
Sport Coach — Web UI (Streamlit, GEARCHIVEERD in Fase 5)

    streamlit run legacy_streamlit/app.py

Bevroren sinds de React PWA (web/) alle flows dekt; zie
...
# Repo-root + eigen map op sys.path (gearchiveerd: woont in legacy_streamlit/)
...
# Swap-logica is geëxtraheerd naar core/swap_service.py (Fase 3) zodat
# de FastAPI-laag dezelfde flow gebruikt — geen duplicatie.
from core import swap_service as _swap_svc

_resolve_phase_tss_range = _swap_svc.resolve_phase_tss_range
predict_week_tss = _swap_svc.predict_week_tss
compute_ideal_tss = _swap_svc.compute_ideal_tss
...
def perform_instant_swap(event: dict, category: str, ftp: int = 290,
    ...
    """
    # Kern (optie kiezen + intervals.icu update + week-check) leeft in
    # core/swap_service.py — gedeeld met de FastAPI-laag.
    swap_result = _swap_svc.perform_swap(
        event, category, ftp=ftp, ideal_tss=ideal_tss,
        matched=matched, phase_tss_range=phase_tss_range,
    )
    if not swap_result["ok"]:
        st.session_state["swap_flash"] = {"ok": False, "msg": swap_result["message"]}
        return None
    chosen = swap_result["chosen"]
    ...
    # Success flash + undo payload. Message + week-check komen uit de service;
    # alleen de TP-propagatie-status is Streamlit-specifiek.
    st.session_state["swap_flash"] = {
        "ok": True,
        "msg": f"{swap_result['message']}{tp_propagation_msg}{swap_result['phase_warning']}",
        "undo": swap_result["undo"],
    }
    return chosen
...
if week_offset >= 0:
    ...
    with st.expander("Beschikbaarheid", expanded=_avail_default_open):
        ...
        try:
            from core.replan import event_duration_min as _ev_dur_min
            ...
            for _d_iso, _av_val in _current_avail.items():
                ...
                _used = sum(_ev_dur_min(e) for e in _day_evs)
                if _used > _av_val:
        ...
        if _new_avail is not None:
            ...
            st.cache_data.clear()
            try:
                from core import planner_v2_enabled as _v2_on

                if _v2_on():
                    # Planner v2: solver opnieuw draaien met huidige
                    # placements als referentie — minimale verschuiving
                    # wint vanzelf via de verplaatsingsterm.
                    from core import replan as _replan
                    _res = _replan.replan_on_availability_change(selected_monday)
                    if _res["moved"]:
                        _msg = (f"{len(_res['moved'])} sessie(s) verschoven "
                                f"— plan behouden.")
                        if _res.get("errors"):
                            _msg += f" ⚠ {len(_res['errors'])} fout(en)."
                        st.success(_msg)
                        st.session_state["shift_flash_moves"] = _res["moved"]
                    elif _res["needs_replan"]:
                        import plan_week as _pw
                        _pw.run(selected_monday, dry_run=False)
                        st.success(f"Week opnieuw gepland ({_res.get('diag', '')}).")
                    else:
                        st.info(f"Beschikbaarheid opgeslagen — {_res.get('diag', '')}.")
                else:
                    # Legacy pad (PLANNER_V2=0): geen automatische shift meer;
                    # gebruik de 'Plan deze week opnieuw'-knop bij overflow.
                    st.info("Beschikbaarheid opgeslagen. Gebruik 'Plan deze "
                            "week opnieuw' als het plan niet meer past.")
